...
(function (root, factory) {
  ...
  // to state. Late = past cadence, not yet past the stale threshold.
  //
  // Gap E fix (WARROOM-READPLANE-001, prior-audit-confirmed 2026-08-26): the
  // stale threshold is the value Shiv ruled per tab in DECISION GATE 5c,
  // recorded verbatim as `stale_at_seconds` in warroom/cadence-registry.json —
  // never an assumed cadence×2 formula. `staleAtSeconds` is that registry
  // value, passed in by the caller (api/state/*.js, api/stream/*.js read it
  // from cadenceRegistry.tabs[TAB].stale_at_seconds). cadence×2 remains only
  // as a fallback for a caller that hasn't supplied the registry value, so
  // this stays backward compatible rather than silently going stale-blind.
  function computeFreshness(computedAtIso, cadenceSeconds, now, staleAtSeconds) {
    ...
    var staleThreshold = (staleAtSeconds !== undefined && staleAtSeconds !== null)
      ? staleAtSeconds
      : cadenceSeconds * 2;
    if (ageSeconds > staleThreshold) return FRESHNESS.STALE;
  ...
  function renderTile(payload, cadenceSeconds, now, staleAtSeconds) {
    ...
    var freshness = computeFreshness(payload.computed_at, cadenceSeconds, nowDate, staleAtSeconds);
    // Gap D fix (WARROOM-READPLANE-001, prior-audit-confirmed 2026-08-26): a
    // cold-start cache miss (fetchTabBlob rejects, no cache built yet) renders
    // an errored payload with no computed_at — it never obtained a reading at
    // all. computeFreshness's "nothing to compare against yet" default is
    // FRESH, which is correct for e.g. a genuine na/zero payload but is a
    // fabricated-currency contradiction (C1/C4) for an error: a tile cannot
    // be simultaneously ERROR and fresh. Floor it to STALE — the honest "we
    // have nothing" signal — instead of claiming a reading that never happened.
    if (payload.state === 'error' && !payload.computed_at) {
  ...
  function computeBreachFreshness(computedAtIso, cadenceSeconds, now, staleAtSeconds) {
    var f = computeFreshness(computedAtIso, cadenceSeconds, now, staleAtSeconds);
  ...
  function renderBreachFields(tab, tileIds, cadenceSeconds, now, staleAtSeconds) {
    ...
    tileIds.forEach(function (tileId) {
      ...
      if (cachedField) {
        var out = {
          ...
          freshness: computeBreachFreshness(cachedField.computed_at, cadenceSeconds, nowDate, staleAtSeconds),
