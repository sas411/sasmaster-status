...
(function (root) {
  ...
  /**
   * toEtFuture(ts, now) -> {display, offset, state, reason?}
   * WARROOM-NOWPLANE-001 addition — toEt()'s clock-skew gate exists because
   * every OTHER caller renders an event that claims to have already
   * happened; a future ts there is a defect. `next_fire` is the one
   * legitimately-future timestamp on the whole board (a computed schedule
   * slot), so it needs the same ET/offset formatting without being flagged
   * as skew. This does NOT weaken toEt()'s own gate — it is a second named
   * function for the one caller whose "future" is expected, not a bypass
   * added to toEt() itself. Still routes through the same ICU-based
   * _etParts()/_fmtClock() this module already uses (C6: one formatter).
   */
  function toEtFuture(ts, now) {
    var d = _coerce(ts);
    if (!_isFinite(d)) return { state: 'error', reason: 'invalid timestamp' };
    var p = _etParts(d);
    return {
