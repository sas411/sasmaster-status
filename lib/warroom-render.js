...
(function (root, factory) {
  ...
  // ---- Slack shortcode conversion (ORCH-WARROOM-FIX-001 finding 5) ----
  // Agent log lines and Slack-mirrored feed text are authored with Slack's
  // ":shortcode:" emoji syntax (Slack itself renders these client-side; War
  // Room just displays the raw log/API text, so the literal ":rotating_light:"
  // was reaching the DOM unconverted). Small, deliberately non-exhaustive map
  // of the shortcodes actually observed in SaSMaster agent/ops logs — extend
  // as new ones show up rather than pulling in a full emoji-shortcode library
  // for one field. Never touches text that isn't a real ":word:" shortcode
  // (the regex requires at least one letter so timestamps like "12:00:00"
  // are never matched).
  var SLACK_SHORTCODES = {
    rotating_light: '🚨',           // 🚨
    warning: '⚠️',                  // ⚠️
    white_check_mark: '✅',               // ✅
    x: '❌',                              // ❌
  ...
  var SHORTCODE_RE = /:([a-z0-9_+-]*[a-z_][a-z0-9_+-]*):/gi;
  function convertSlackShortcodes(text) {
    if (text == null) return text;
    return String(text).replace(SHORTCODE_RE, function (m, name) {
      var emoji = SLACK_SHORTCODES[name.toLowerCase()];
      return emoji != null ? emoji : m; // unknown shortcode: leave as-is, never drop text
    });
  }
  ...
  function formatTelemetryEvent(ev, id, clock) {
    try {
      ...
      var subject = convertSlackShortcodes(String(ev.text != null ? ev.text : '')).trim();
  ...
  return {
    ...
    convertSlackShortcodes: convertSlackShortcodes,
